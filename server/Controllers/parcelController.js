...
const parcel = {
  ...
  `;
    ...
    if (
      ...
      return res
        .status(400)
        .send({ message: "complete all fields to proceed" });
    }

    if (
      req.body.pickup.toLowerCase() === req.body.pickup.toUpperCase() ||
      req.body.destination.toLowerCase() ===
        req.body.destination.toUpperCase() ||
      req.body.receiver.toLowerCase() === req.body.receiver.toUpperCase()
    ) {
    ...
    if (
      req.body.weight.toLowerCase() !== req.body.weight.toUpperCase() ||
      req.body.receiver_phone.toLowerCase() !==
        req.body.receiver_phone.toUpperCase()
    ) {
      return res
        .status(400)
        .send({ message: "CHECK: weight and receiver_phone must be a number" });
    }
...
`;
  ...
  `;
    ...
    if (req.body.email !== req.params.id) {
      return res.status(400).send({
        message: "the specified user in url is not the one logged in"
      });
    }
...
`;
    ...
    const data = [req.body.presentLocation, req.params.id];

    if (!req.body.presentLocation) {
      return res
        .status(400)
        .send({ message: "complete the presentLocation fields to proceed" });
    }
...
`;
    ...
    if (!req.body.status) {
      return res
        .status(400)
        .send({ message: "complete the status fields to proceed" });
    }
  ...
  async destination(req, res) {
    ...
    if (!req.body.destination) {
      return res
        .status(400)
        .send({ message: "complete the destination field to proceed!!" });
    }
    ...
    `;
      ...
      const data2 = [req.body.destination, req.params.id];
  ...
  `;
    ...
    const hashPassword = bhelp.hashPassword(req.body.password);
    ...
    if (
      req.body.first_name.toLowerCase() === req.body.first_name.toUpperCase() ||
      req.body.last_name.toLowerCase() === req.body.last_name.toUpperCase() ||
      req.body.town.toLowerCase() === req.body.town.toUpperCase() ||
      req.body.email.toLowerCase() === req.body.email.toUpperCase()
    ) {
      return res.status(400).send({
        message: "CHECK: first_name, last_name, town, and email must be a word"
      });
    }

    if (
      req.body.phone_number.toLowerCase() !==
      req.body.phone_number.toUpperCase()
    ) {
      return res
        .status(400)
        .send({ message: "CHECK: phone_number must be a number" });
    }
  ...
  async signin(req, res) {
    ...
    try {
      ...
      return res
        .status(200)
        .send({ user: rows[0].email, name: rows[0].first_name, token });
    } catch (error) {
      return res.status(400).send(error.message);
    }
  },
...
`;
    ...
    const data = ["CANCELED", req.params.id];
...
`;
    ...
    const { rows } = await query(sql2, data2);
    ...
    if (req.body.email != rows[0].email) {
      return res.status(400).send({
        message: "you do not have a parcel with the specified id in the URL"
      });
    }
  ...
  async delete(req, res) {
    const sql = "DELETE FROM parcels WHERE id = $1";
    const data = [req.params.id];

    const sql2 = "SELECT * FROM parcels WHERE id = $1";

    try {
      const { rows } = await query(sql2, data);
  ...
  async getAllUserParcels(req, res) {
    const sql = "SELECT * FROM parcels WHERE email = $1";
    const data = [req.body.email];
